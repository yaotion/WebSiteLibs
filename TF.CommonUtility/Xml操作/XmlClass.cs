...
namespace TF.CommonUtility
{
    ...
    public class XmlClass
    {
        ...
        public static string[] ReadAllChildallValue(string path, string node)
        {
            int i = 0;
            string[] str = { };
            XmlDocument doc = XMLLoad(path);
        ...
        public static bool CDataToXmlFile(DataTable dt, string xmlFilePath)
        {
            if ((dt != null) && (!string.IsNullOrEmpty(xmlFilePath)))
            {
                ...
                try
                {
                    ...
                    sw.Write(ucode.GetString(temp).Trim());
                    sw.Close();
                    return true;
                }
        ...
        public static DataSet CXmlFileToDataSet(string xmlFilePath)
        {
            if (!string.IsNullOrEmpty(xmlFilePath))
            {
                ...
                try
                {
                    string strXml = File.ReadAllText(path, Encoding.UTF8).Trim();
                    XmlDocument xmldoc = new XmlDocument();
                    //根据地址加载Xml文件
